...
namespace Meshmakers.Octo.Communication.EdgeAdapter.Demo.Tests.Services;

public class AdapterEdgeDemoServiceTests
{
    private readonly ILogger<AdapterEdgeDemoService> _logger;
    private readonly IPipelineRegistryService _pipelineRegistryService;
    private readonly IEventHubControl _eventHubControl;
    private readonly AdapterEdgeDemoService _sut;

    public AdapterEdgeDemoServiceTests()
    {
        _logger = A.Fake<ILogger<AdapterEdgeDemoService>>();
        _pipelineRegistryService = A.Fake<IPipelineRegistryService>();
        _eventHubControl = A.Fake<IEventHubControl>();
        _sut = new AdapterEdgeDemoService(_logger, _pipelineRegistryService, _eventHubControl);
    }
    ...
    public async Task StartupAsync_RegistersPipelines_AndStartsEventHub()
    {
        ...
        var config = new AdapterConfigurationDto(new RtEntityId("System.Communication/EdgeAdapter@6760711ec4ff02221e0b532d"), null, pipelines);
