...
public class DemoNodeTests
{
    ...
    {
        // Arrange
        var config = new DemoNodeConfiguration { MyMessage = "Mesh Test Message" };
        ...
        A.CallTo(() => _dataContext.SetValueByPath(
            ...
            "Mesh Test Message")).MustHaveHappenedOnceExactly();
    }
    ...
    [Fact]
    public async Task ProcessObjectAsync_UsesCustomTargetPath()
    {
        // Arrange
        var config = new DemoNodeConfiguration
        {
            MyMessage = "Custom Path",
            TargetPath = "$.output.result"
        };
        A.CallTo(() => _nodeContext.GetNodeConfiguration<DemoNodeConfiguration>()).Returns(config);
        ...
        A.CallTo(() => _dataContext.SetValueByPath(
            ...
            config.TargetValueKind,
            config.TargetValueWriteMode,
            "Custom Path")).MustHaveHappenedOnceExactly();
    }
}
